...
#if DEMO || STAGING
        navigationService.simulationSpeedMultiplier = 2
#endif
        let options = NavigationOptions(styles: [CustomDayStyle()], navigationService: navigationService, predictiveCacheOptions: PredictiveCacheOptions())
  
        let bottomVC = BottomBannerViewController()
        options.bottomBanner = bottomVC

        let topBannerVC = TopBannerViewController()
        options.topBanner = topBannerVC
        
        let navigationViewController = CustomNavigationVC(for: navigationService.router.indexedRouteResponse.routeResponse, routeIndex: 0, routeOptions: routeOptions, navigationOptions: options)
        navigationViewController.routeLineTracksTraversal = true
        bottomVC.delegate = navigationViewController
